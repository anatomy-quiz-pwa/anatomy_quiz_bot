...
def get_demo_question(user_level: int) -> Dict:
    ...
    demo_questions = {
        ...
        4: {
            'id': 4,
            'question': '中樞神經系統包括？',
            'options': ['大腦和脊髓', '大腦和周圍神經', '脊髓和周圍神經', '只有大腦'],
            'correct_answer': 0,
            'explanation': '中樞神經系統包括大腦和脊髓。',
            'level': 4,
            'category': '神經系統'
        },
        5: {
            ...
            'correct_answer': 2,
            'explanation': '肺泡是氣體交換的主要場所，氧氣和二氧化碳在這裡交換。',
            'level': 5,
            'category': '呼吸系統'
        }
    }
...
def update_user_stats(user_id: str, is_correct: bool) -> bool:
    ...
    try:
        if supabase is None:
            # 演示模式：只更新session中的數據
            if 'demo_stats' not in session:
                session['demo_stats'] = {'correct': 0, 'total': 0, 'level': 1}
            
            session['demo_stats']['correct'] += 1 if is_correct else 0
            session['demo_stats']['total'] += 1
            
            # 每答對5題升級一次
            if is_correct and session['demo_stats']['correct'] % 5 == 0:
                session['demo_stats']['level'] = min(14, session['demo_stats']['level'] + 1)
...
def get_leaderboard_data() -> List[Dict]:
    ...
    try:
        if supabase is None:
            # 演示模式：返回模擬數據
            return [
                {'rank': 1, 'name': '蘇同學', 'score': 450, 'correct': 45, 'total': 50, 'level': 14},
                {'rank': 2, 'name': '寶同學', 'score': 370, 'correct': 37, 'total': 45, 'level': 13},
                {'rank': 3, 'name': '小明', 'score': 270, 'correct': 27, 'total': 35, 'level': 7},
                {'rank': 4, 'name': '小華', 'score': 200, 'correct': 20, 'total': 30, 'level': 6},
                {'rank': 5, 'name': '小美', 'score': 150, 'correct': 15, 'total': 25, 'level': 4}
            ]
...
def demo():
    """演示模式登入"""
    # 創建演示用戶
    demo_user_id = f"demo_user_{secrets.token_hex(8)}"
    session['user_id'] = demo_user_id
    session['display_name'] = "演示用戶"
    session['demo_mode'] = True
    
    # 初始化演示統計
    session['demo_stats'] = {'correct': 0, 'total': 0, 'level': 1}
    
    logger.info(f"✅ 演示用戶 {demo_user_id} 登入")
...
def game():
    ...
    if session.get('demo_mode'):
        demo_stats = session.get('demo_stats', {'correct': 0, 'total': 0, 'level': 1})
        user_stats = {
            'correct': demo_stats['correct'],
            'total': demo_stats['total'],
            'level': demo_stats['level'],
            'score': demo_stats['correct'] * 10
        }
        nickname = session.get('display_name', '演示用戶')
    else:
...
def start_quiz():
    ...
    # 獲取用戶等級
    if session.get('demo_mode'):
        user_level = session.get('demo_stats', {'level': 1})['level']
    else:
        user_stats = get_user_stats(user_id)
