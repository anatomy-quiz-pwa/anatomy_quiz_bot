...
## ✅ 已建立的 Vercel Functions

已成功建立 4 個 Vercel Functions 檔案：

### 1. OIDC 登入啟動
**檔案**: `api/auth/line/login.ts`
**功能**: 啟動 LINE OIDC 登入流程（PKCE）
**測試**: `https://anatomy-quiz-bot.vercel.app/api/auth/line/login`

### 2. OIDC 回調處理
**檔案**: `api/auth/line/callback.ts`
**功能**: 處理 LINE 授權回調，換取 id_token，驗簽後設定 Session
**測試**: 由 LINE 自動調用

### 3. LIFF 驗證
**檔案**: `api/auth/line/verify.ts`
...
### ✅ 專案結構檢查
- [x] 專案不是 Next？ → 用 Vercel Functions 版本
- [x] 檔案確實位於根目錄 `api/`？ → 不是 `src/api`、不是 `pages/api`
- [x] 使用 Vercel Functions 類型 (`VercelRequest`, `VercelResponse`)

### 🚀 部署檢查
- [ ] 重新部署後，在 Vercel Deployment 面板的「Functions」看到 4 支函式
- [ ] 打開 `https://anatomy-quiz-bot.vercel.app/api/auth/line/login` → 應該 302 到 LINE，而不是 404

### 🔧 設定檢查
- [ ] LINE Developers 的 Callback URL 是否仍指向：
  `https://anatomy-quiz-bot.vercel.app/api/auth/line/callback`（一字不差）
- [ ] Vercel env 是否都有：`LINE_LOGIN_CHANNEL_ID`、`LINE_LOGIN_CHANNEL_SECRET`、`APP_SESSION_SECRET`、`SUPABASE_URL`、`SUPABASE_SERVICE_KEY`（且已 Redeploy）

## 🧪 測試流程

### 1. 測試 OIDC 登入
1. 開啟 `https://anatomy-quiz-bot.vercel.app/api/auth/line/login`
2. 應該跳轉到 LINE 授權頁面（不再是 404）
...
1. **確認檔案結構**（已完成）
2. **設定環境變數**（使用上面的清單）
3. **設定 LINE Console**（使用上面的設定）
4. **部署到 Vercel**：
   ```bash
   vercel --prod
   ```
5. **檢查 Functions 面板**：在 Vercel Dashboard → Deployments → Functions 看到 4 支函式
6. **測試所有 API 端點**

## 🔍 故障排除

### 404 錯誤
- 確認 Vercel Functions 檔案路徑正確（`api/` 根目錄）
- 確認檔案名稱和函數名稱正確
- 重新部署專案
- 檢查 Vercel Functions 面板是否有函式
...
- ✅ LIFF 應用程式正常運作
- ✅ Vercel Functions 面板顯示 4 支函式

## 🎉 完成！

所有 Vercel Functions 都已建立完成，現在可以部署並測試 LINE Login A/B 兩條路徑的完整功能！

**關鍵差異**：使用 `api/` 根目錄的 Vercel Functions，不依賴 Next.js Router，Vercel 會自動當 Serverless Functions 部署。
