...
def test_alternative_solution():
    ...
    try:
        ...
        # 檢查是否包含替代解決方案標識符
        if "ALTERNATIVE_SOLUTION_V4_2025_09_17" in response.text:
            print("🎉 替代解決方案成功！Render 正在使用我們的文件")
            return True
        else:
            print("❌ 不包含替代解決方案標識符，Render 沒有使用我們的文件")
            return False
...
def test_webhook_with_alternative_solution():
    """測試 webhook 替代解決方案"""
    print("\n🔍 測試 webhook 替代解決方案...")
    ...
    try:
        ...
        # 檢查是否包含替代解決方案標識符
        if "ALTERNATIVE_SOLUTION_V4_2025_09_17" in response.text:
            print("🎉 webhook 替代解決方案成功！")
            return True
        else:
            print("❌ webhook 不包含替代解決方案標識符")
            return False
...
def wait_for_deployment():
    """等待部署完成"""
    print("⏳ 等待部署完成...")
    print("💡 請確保您已經將替代解決方案上傳到 Render 並重新部署")
...
def main():
    ...
    # 測試根路徑
    root_ok = test_alternative_solution()
    
    # 測試 webhook
    webhook_ok = test_webhook_with_alternative_solution()
